...
function getWotd(req, res, next) {
  ...
  fbApiReq = https.get('https://graph.facebook.com/' +
    process.env.FB_APP_TARGET_PAGE + '?fields=posts' +
...
function init(server, logger) {
    logger.info('Initializing wotd controller...');

    server.get('/wotd', getWotd);

    logger.info('wotd controller initialization done!');
}
